...
- **Launcher: symlink bypass of the staging-root refusal — fixed (INV-BASE-01).**
  ...
  *through a symlink* (`/tmp/x -> $HOME`, say) presented a benign-looking string, sailed past
  ...
  to its **physical** location (`physicalPrefix` → `expandFilename`) before the root/drive/home
  checks, and the reaper refuses a target that has since become a symlink. Found by a "what
  about smuggling in path traversals or symlinks?" question during review; proven bypassable
  (`/tmp/link -> $HOME` returned ACCEPTED), then fixed with a claiming test whose red-path was
  walked. Compiles on release / haruDev / `nim check --os:windows`.
