...
|---|---|---|
| `INV-TIER-04` | `--free-threaded` is not a silent no-op at `default`/`thin` | §3.4, step 5 |
| `INV-SUPPLY-12` | the `.tar.zst` path does not escape the pin + member sanitizer | §3.3, step 2 |
...
This document also demonstrates the failure it warns about. It was written on 2026-09-10 claiming
`INV-TIER-03` and `INV-SUPPLY-11`, sat unmerged on a branch, and by the time it landed both ids
had been taken on `main` by unrelated claims — foreign-target objects, and the
python-build-standalone pin. They are `INV-TIER-04` and `INV-SUPPLY-12` here. An id reserved on a
branch is not reserved; it is reserved when it is on `main`, and a doc that sits still is a doc
whose numbers rot.
...
2. zstd + `python/install/**` extraction in `archives.py`, behind `safe_extract_tar`
   (`INV-SUPPLY-12`). The largest piece — do it before anything depends on it.
...
5. `python_request` + Nim + `UV_PYTHON` (`INV-TIER-04`). Until this lands, **refuse
