...
| T-015 | 2026-09-14 | lotek→haru | deterministic markdown report + `replay` + `author` | landed | `tests/busybody/report.py`, `replay.py`, `author.py`. haru-pack's `report.txt` is flat text; lotek's is a no-LLM deterministic markdown report, plus a `replay` that reconstructs a failing k-set and an `author` that generates a corpus script covering what prior runs missed |
| T-016 | 2026-09-14 | lotek→haru | forensic bundles: collect the state that EXPLAINS a finding, not only the record | landed | `tests/busybody/bundle.py`. lotek collects py-spy, `pg_stat_activity`, screenshots. The haru-pack analogue is process tree, open FDs, staged-dir listing, scratch usage |
...
| T-020 | 2026-09-14 | (neither) | shrinking (`ddmin`) over the fired trait stack | **declined** | **spiked and rejected for this axis** — see `docs/BUSYBODY-SHRINKING-SPIKE.md`. At the stack sizes haru-pack actually runs (k=1/2/3, the only three composed invocations the docs describe) ddmin costs MORE than exhaustive search and returns the same answer: 10 calls against 4 at k=3. The small search space that made this repo look like the better place to spike it is exactly why it does not pay here. Two things worth keeping from the spike: ddmin never under-approximates under a flaky oracle (it inflates instead, which is the safe direction), and `--compose-only` already forces every trait to fire, so an oracle built on it runs at p=1 |
| T-022 | 2026-09-14 | (neither) | shrinking over the FIXTURE set, not the trait stack | proposed | the redirect that came out of T-020's spike. `--fixtures top25` is a k=25 problem, which is where ddmin's advantage over exhaustive search becomes real (105 calls against 598 in the trial), and an oracle call there is running an existing binary rather than a 54-second build. Same argument for the 80-case set on a cross-cutting regression |
